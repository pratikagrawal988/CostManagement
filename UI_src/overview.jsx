...
// ─────────────────────────────────────────────────────────────────
function OverviewScreen({ chartMode = 'area' }) {
  const days = 14;
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1.55fr 1fr', gap: 10 }}>
            ...
            <div className="card" style={{ padding: '12px 14px 4px', display: 'flex', flexDirection: 'column', minHeight: 260 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 6 }}>
                ...
                <div style={{ flex: 1 }} />
                <Legend items={[
                  { color: 'var(--aws)', label: 'AWS', value: fmtUSD(aws.reduce((a,b)=>a+b,0), {compact:true}) },
                  { color: 'var(--gcp)', label: 'GCP', value: fmtUSD(gcp.reduce((a,b)=>a+b,0), {compact:true}) },
                  { color: 'var(--azure)', label: 'Azure', value: fmtUSD(az.reduce((a,b)=>a+b,0), {compact:true}) },
                ]} />
              </div>
              <StackedArea
                width={760}
...
function ForecastCard({ forecast }) {
  // Forecast cone visualization: realized line then forecast cone widening.
  ...
  const x = (i, n) => padL + (i / (n - 1)) * w * 0.5; // realized in first half
...
// ─────────────────────────────────────────────────────────────────
// SANKEY — providers → services → teams. Simple custom rendering.
// ─────────────────────────────────────────────────────────────────
function Sankey() {
  const W = 760, H = 168;
  const left = [
  ...
  // Links: provider→service, service→team. Manually crafted weights.
  const lp = [
  ...
  // Build link offsets within each node
