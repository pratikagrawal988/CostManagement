...
def export_focus_csv(
    ...
    for row in results:
        writer.writerow({
            ...
            "InvoiceIssuer": row.invoice_issuer_name,
            "ServiceName": row.service_name,
            "ServiceCategory": row.service_category,
            "SKU": row.sku_id,
            "Region": row.region_name or row.region_id,
            "UsageQuantity": row.usage_quantity,
            "UsageUnit": row.usage_unit,
            "UnitPrice": row.list_unit_price,
            "BilledCost": row.billed_cost,
            "EffectiveCost": row.effective_cost,
            "Currency": row.currency,
            "ResourceId": row.resource_id,
            "Team": row.x_team,
            "CostCenter": row.x_cost_center,
            "Environment": row.x_environment,
            "Tags": row.tags,
        })
