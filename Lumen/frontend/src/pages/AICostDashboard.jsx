...
import {
  Sidebar, Topbar, FilterBar, KPI, Spark, StackedArea, Ico, Tag, fmtUSD, pct, genCostSeries,
...
export default function AICostDashboard({ onNavigate, chartMode = 'area' }) {
  const days = 14;
  ...
  return (
    <div className="lumen">
      <Sidebar active="ai" onNavigate={onNavigate} />
...
// 142 seats; show as 24-col grid. Color = activity level.
// ─────────────────────────────────────────────────────────────────
function SeatHeatmap() {
  let s = 11;
  const rnd = () => { s = (s * 9301 + 49297) % 233280; return s / 233280; };
  const cells = Array.from({ length: 142 }, () => {
    const r = rnd();
